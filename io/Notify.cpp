...
void Notify::Close()
{
	if (inotify_fd_ != -1 && (pthread_mutex_lock(&mutex) == 0)) {
		close(inotify_fd_);
		inotify_fd_ = -1;
...
AutoRemoveWatch::AutoRemoveWatch(io::Notify &notify, cint dir_watch_fd):
	notify_(notify), wd_(dir_watch_fd)
{
	MutexGuard guard = notify_.guard();
	cint count = notify_.watches.value(wd_, 0);
...
{
	MutexGuard guard = notify_.guard();
	cint remaining = notify_.watches.value(wd_, 0) - 1;
	
	if (remaining > 0) {
		notify_.watches[wd_] = remaining;
	} else {
		notify_.watches.remove(wd_);
		
		if (remaining == 0) {
			cint status = inotify_rm_watch(notify_.inotify_fd_, wd_);
			if (status != 0)
