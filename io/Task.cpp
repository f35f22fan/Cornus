...
void Task::CopyFileToDir(const QString &file_path, const QString &in_dir_path)
{
	...
	{
		data_.ChangeState(TaskState::InternalError);
		return;
	}
	/// PROGRESS>>
	...
	if (S_ISDIR(mode))
	{
		...
		for (const auto &name: names)
		{
			...
			if (data_.GetState() == TaskState::InternalError)
				return;
		}
...
//mtl_trace("\"%s\", name: \"%s\"", qPrintable(new_dir_path), qPrintable(filename));
	if (out_fd == -1)
	{
		mtl_trace("out_fd == -1");
		return;
	...
	{
		isize count = copy_file_range(input_fd, &in_off, out_fd, &out_off, chunk, 0);
		if (count == -1)
		{
			if (errno == EAGAIN)
				continue;
...
void Task::StartIO()
{
	...
	if (ops_ == (MessageType)Message::DeleteFiles) {
		...
		if (TryAtomicMove())
		{
			...
			CopyFiles();
			if (data_.GetState() & TaskState::Continue)
			{
				DeleteFiles();
			}
		}
	...
	const mode_t mode, const i64 file_size, QString &dest_path)
{
	...
	while (true)
	{
		...
		if (Error == EEXIST)
		{
			...
			switch (answer) {
			case ActUponAnswer::Retry: break;//continue;
