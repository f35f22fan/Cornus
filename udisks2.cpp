    ...
    QObject(parent)
{
    ...
    for (QString block: blockDevices()) {
        addBlock(block);
    }
...
void UDisks2::dbus_interfaceRemoved(const QDBusObjectPath &path, const QStringList &interfaces)
{
    ...
    if (path.path().startsWith("/org/freedesktop/UDisks2/block_devices")) {
        if (interfaces.contains("org.freedesktop.UDisks2.Block")) {
            removeBlock(node);
            Q_EMIT blockDeviceRemoved(node);
        }
        if (interfaces.contains("org.freedesktop.UDisks2.Filesystem")
                && blocks_.contains(node)) {
...
void UDisks2Block::addFilesystem()
{
    ...
    Q_EMIT filesystemAdded(name);
}

void UDisks2Block::removeFilesystem()
{
    Q_EMIT filesystemRemoved(name);
    if (fs) delete fs;
    fs = NULL;
}
...
void UDisks2Block::self_propertiesChanged(const QString &interface, const QVariantMap &changedProp, const QStringList &invalidatedProp)
{
    if (interface == "org.freedesktop.UDisks2.Block") {
        update();
        Q_EMIT changed(name);
    } else if (interface == "org.freedesktop.UDisks2.Filesystem") {
        updateFilesystem();
        Q_EMIT filesystemChanged(name);
	} else {
		mtl_info("Unhandled interface: %s", qPrintable(interface));
	}
}
